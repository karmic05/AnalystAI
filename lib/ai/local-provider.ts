...
import { linreg, mean, sum } from "@/lib/analysis/stats";
...
export function writeReport(ctx: AIContext): string {
  ...
  const risks = ins.filter((i) => /risk|decline|outlier|skew|concentrat|duplicate|missing/i.test(i.title + i.impactReason));
  const opps = ins.filter((i) => /lead|grow|top|driver|strong|peak/i.test(i.title + i.impactReason));

  const md = `# Executive Report — ${ds.name}

Generated by AnalystAI · ${new Date().toDateString()}

## 1. Executive Summary
This report covers ${ds.rowCount.toLocaleString()} records across ${ds.columns.length} columns${metric ? `, anchored on ${titleCase(metric.name)}` : ""}. ${metric ? `Total ${titleCase(metric.name)} is ${fmt(sum(series), { compact: true })} at an average of ${fmt(mean(series), { compact: true })} per record.` : ""} ${profile.duplicateRows ? `Note: ${profile.duplicateRows} duplicate rows were detected and should be resolved before relying on totals.` : "Data completeness is high."}

## 2. Key Insights
${top.map((i, idx) => `### ${idx + 1}. ${i.title}\n${i.summary}\n\n- **Confidence:** ${Math.round(i.confidence * 100)}%  ·  **Impact:** ${i.impact}\n- **Action:** ${i.action}`).join("\n\n")}

## 3. KPI Overview
${metric ? `- Total ${titleCase(metric.name)}: ${fmt(sum(series), { compact: true })}\n- Average per record: ${fmt(mean(series), { compact: true })}\n- Records: ${ds.rowCount.toLocaleString()}\n- Completeness: ${pct(profile.completeness)}` : "- No numeric metric detected."}

## 4. Forecast
${fc ? fc.summary.replace(/^Projected/, "Projected headline metric: projected") : "No time series available for forecasting."}

## 5. Risks
${risks.length ? risks.slice(0, 3).map((i) => `- ${i.title} — ${i.impactReason}`).join("\n") : "- No material risks flagged by the automated analysis."}

## 6. Opportunities
${opps.length ? opps.slice(0, 3).map((i) => `- ${i.title} — ${i.action}`).join("\n") : "- Review the segmentation tab to surface growth opportunities."}

## 7. Recommendations & Action Plan
${top.slice(0, 3).map((i, idx) => `${idx + 1}. ${i.action}`).join("\n")}

---
*This report was generated automatically from the uploaded data. Confidence scores reflect sample size and effect strength; validate high-stakes decisions with segment-level checks.*
`;
  return md;
}
