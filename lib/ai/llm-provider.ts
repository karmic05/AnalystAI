...
const TASK_INSTRUCTIONS: Record<AITask, string> = {
  ...
  "report-writing": "Write a crisp, board-ready executive brief in Markdown. Use ONLY the provided numbers — never invent figures. Be concise: short bullets, not paragraphs; lead each bullet with the point. Sections (in order, '## ' headings): 'At a glance' (6 one-line KPI bullets with **bold** labels), 'TL;DR' (max 3 bullets, the sharpest takeaways), 'Deep analysis' (only if sentiment/pareto/cohort/keyword data is given), 'Key findings' (max 5 bullets, each with impact + confidence in italics), 'Forecast' (2 bullets), 'Risks' (max 3 bullets), 'Opportunities' (max 3 bullets), 'Recommended actions' (numbered, max 3, imperative verbs). Title as '# <Dataset> — Executive Brief'. No filler, no hedging, no repetition. Every claim must be backed by a number from the context.",
