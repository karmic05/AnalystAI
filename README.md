...
- **Data ingest** — CSV, JSON, paste, and two built-in samples (SaaS revenue + product reviews). Schema detection, type inference, missing-value & duplicate detection.
...
- **Advanced analyses (prompt-triggered)** — a deterministic **Advanced** tab that runs on demand from a plain-text prompt: **sentiment analysis** (AFINN-style lexicon scoring with negation/intensifier handling → positive/negative/neutral distribution + example quotes), **text / keyword themes** (word-frequency top terms on a free-text column), **Pareto / 80-20** (vital-few vs long-tail concentration with a cumulative curve), and **cohort retention** (first-period customer cohorts × retention over time). Each also emits an insight and feeds the LLM context.
...
- **AI orchestration layer** — task-based routing (`reasoning`, `sql`, `cleaning`, `chart-explanation`, `forecast-interpretation`, `report-writing`, `kpi-generation`, `chat`) with a per-task model map. Every task prefers **Groq** (free tier, `GROQ_API_KEY`, OpenAI-compatible) for LLM-authored prose, and falls back to the deterministic **local provider** when no key is set or a call errors. The engine always computes the numbers and passes them as grounded context — chat and reports read as Groq prose, but figures can't be hallucinated. Set `GROQ_API_KEY` in `.env.local` to enable it.
- **Design system** — paper marketing × terminal studio. Off-white **paper-textured** marketing with a modern-journal type mix (Bricolage Grotesk display, Dancing Script cursive accents, Source Serif 4 body, Space Mono terminal); a classic **dark-terminal** Studio (near-black, green/amber/cyan, scanlines, `>_` command cells). Route-aware theme (paper on `/`, terminal on `/studio`) with a per-surface toggle.
- **Prompt-driven focus** — a prompt in the brief bar **or** chat is parsed into an intent (`metric`, `dimension`, `horizon`, `period`) that re-derives Insights, Forecast, EDA, Dashboard and KPI so every section reflects the question; a focus banner shows the active scope and clears on demand.
...
Paper marketing × terminal studio. Marketing pages render on a warm **off-white paper texture** (dot-grid + grain + vignette) with a modern-journal type mix: **Bricolage Grotesk** for headings, the logo and big numbers; **Dancing Script** for cursive accents (the logo `ai`, hero flourish, notebook margin notes, the footer sign-off); **Source Serif 4** for body prose; **Space Mono** for terminal cells, code, data readouts and the `// eyebrow` / `>_` prompt flourishes. The Studio is a classic **dark terminal** — near-black canvas with a faint green grid, scanlined `.terminal-cell` readouts, `>_` command prompts, and green/amber/cyan accents. The two are switched automatically by route (paper on `/`, terminal on `/studio`) and overridable per surface via the toggle; a no-flash inline script sets the theme before paint. Signature accent: a magenta → purple → cyan brand gradient. Reusable classes (`.panel`, `.neu`, `.neu-inset`, `.terminal-cell`, `.btn-neu[-accent]`, `.text-gradient`, `.eyebrow`, `.font-script`, `.grid-bg`, `.radial-glow`, `.scanlines`, `.prompt`, `.cursor`) live in `app/globals.css`; charts share a palette in `lib/viz.ts`.

### Prompt-driven focus

A prompt entered in the brief bar **or** chat is parsed by `parseBrief` into an `AnalysisIntent` (`metric`, `groupBy`, `horizon`, `period`, `analyses`). The analyze page threads that focus into the analysis engine — `generateInsights(dataset, {metric, category})`, `forecast(dataset, {valueColumn, period, horizon})`, `topCorrelations(dataset, limit, anchor)`, and the `pickPrimary*` selectors all accept optional overrides — so the top-level `useMemo` hooks re-derive and **every section (Insights, EDA, Forecast, Dashboard, KPI, Report) reflects the prompt**, not just the chat answer. A focus banner shows the active scope (`▸ focus: revenue by region`) with quick-jump links to the reflected sections and a clear button.
